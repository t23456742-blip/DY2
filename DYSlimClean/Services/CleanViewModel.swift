...
final class CleanViewModel: ObservableObject {
    ...
    /// 抖音查询 v3（直连、无代理）：机型/注册时间/国家/抖音号/名称/状态/在线/版本/抖音版本
    func queryDouyinAccountV3() {
        ...
        log("开始抖音查询 v3（直连无代理）…")
        Task.detached(priority: .userInitiated) { [cleaner] in
            let snap = DouyinAccountQuery.query(cleaner: cleaner)
            await MainActor.run {
                self.isBusy = false
    ...
    /// 提参：沙盒提取 16 参 → `/private/var/mobile/Media/{抖音号}.txt`
    func extractAwemeParams() {
        guard !isBusy else { return }
        isBusy = true
        busyText = "提参中…"
        log("开始提参 → Media/{抖音号}.txt …")
        Task.detached(priority: .userInitiated) { [cleaner] in
            let r = DouyinParamExtractor.extractAndSave(cleaner: cleaner)
            await MainActor.run {
                self.isBusy = false
